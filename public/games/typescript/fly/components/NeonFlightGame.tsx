...
const NeonFlightGame = ({ onExit }: NeonFlightGameProps) => {
  ...
  useEffect(() => {
    ...
    camera.position.set(0, 1.5, 3.5); // Closer Camera
    ...
    const fireWeapon = () => {
      ...
      // ... (Bullet creation logic remains same)
    ...
    const endGame = () => {
      ...
      setTimeout(() => resetToMenu(), 4000); // Does resetToMenu call onExit? User asked to destroy.
    };
    ...
    const resetToMenu = () => {
      // Used by Game Over mostly, or we could redirect to onExit
      document.exitPointerLock();
      gameState.gameActive = false;
      gameState.isPaused = false;
      setCursor(containerRef, "default");
      ...
      uiSystem.hidePause();
      uiSystem.showIntro();
      // If we want to fully restart state, we just show intro.
      // But user said "Quit saia do jogo e destrua".
      // Game Over -> Reset To Menu -> Start Game -> fresh state.
      // Pause -> Quit -> onExit (Unmount).
    };

    // ... (Update loop matches, verify GameLogic etc)
    const updateTrails = () => {
        // ... match original
    ...
    // Expose helpers to window if needed
  ...
  useEffect(() => {
    ...
    const createPlayerMesh = () => {
      // We manually add it to scene via ref, but ref should track the group
      if (playerRef.current) scene.remove(playerRef.current);
      // Re-assigning ref.current is not React-way but fine here since we manage Three scene manually
      const group = new THREE.Group();
      (playerRef as any).current = group; 
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          ...
          <button className="minimal-btn" onClick={() => (window as any).resetToMenu()}>
            QUIT TO MENU
          </button>
        </div>
      </div>
