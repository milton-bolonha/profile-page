...
const TransitionAd: React.FC<TransitionAdProps> = ({ onComplete, direction = 'left' }) => {
  ...
  const handleExit = () => {
    ...
    if (cameraRef.current) {
        // Quick "pull back" or "continue curve"
        // User said: "Coloque a segunda pra sair" (Put the second part to exit).
        // We'll move it further along the path or away.
        const currentPos = cameraRef.current.position;
        gsap.to(currentPos, {
            x: currentPos.x + (direction === 'left' ? 2 : -2), // Rapid exit move
            z: currentPos.z - 2, // Pull back/out
            duration: 1.5, // Slower (Was 0.7)
            ease: "power2.in"
        });
    }
      ...
      (gltf) => {
        ...
        cameraRef.current = camera; // Store for exit handler
         ...
         if (direction === 'left') {
            ...
            // Start at the END position of the Entry (entryX)
            camera.position.set(entryX, camHeight, entryZ);
            camera.lookAt(lookTarget);

            tl.to(camera.position, {
                x: startX, // Move back to Start (Left)
                z: startZ,
                duration: 5,
                ease: "power2.inOut",
                onUpdate: () => camera.lookAt(lookTarget),
